...
fileFun <- "functions.f95"           # implementation of Fortran functions
...
model$setPars(cbind( dx=rep(dx, nx), kf=rep(5., nx), ne=rep(0.17, nx),
  h0=rep(-10, nx), hBed=rep(10, nx), widthBed=rep(0.5*dx, nx), kfBed=rep(5., nx),
  thickBed=rep(0.1, nx), leaky=c(1, rep(0, nx-1)) ))
