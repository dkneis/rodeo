---
title: 'rodeo: Basic Usage and Sample Applications'
...
vignette: |
  %VignetteIndexEntry{rodeo: Basic Usage and Sample Applications} %VignetteEngine{knitr::rmarkdown} %VignetteEncoding{UTF-8}
---
...
$$ \frac{d}{d t}bac = \color{blue}{mu * \frac{sub}{sub+half} * bac} + \color{red}{\frac{flow}{vol}} * (0-bac) $$

$$ \frac{d}{d t}sub = -\frac{1}{yield} * \color{blue}{mu * \frac{sub}{sub+half} * bac} + \color{red}{\frac{flow}{vol}} * (sub_{in}-sub) $$
...
\begin{bmatrix} mu * \dfrac{sub}{sub+half} * bac \\ \dfrac{flow}{vol} \end{bmatrix} *
...
```{r child='examples_0D_streeterPhelpsLike.rmd'}
```

### Bacteria in a 2-zones stirred tank
```{r child='examples_0D_twoZonesStirredTank.rmd'}
```

## One-dimensional models

### Diffusion
```{r child='examples_1D_diffusion.rmd'}
```

### Advective-dispersive transport
```{r child='examples_1D_advectionDispersion.rmd'}
```

### Ground water flow
```{r child='examples_1D_groundwater.rmd'}
```

### Antibiotic resistant bacteria in a river
```{r child='examples_1D_tetracycline.rmd'}
```

## Multi-object models {#examples-multiObject}

### Water-sediment interaction
```{r child='examples_linked_waterSediment.rmd'}
```
