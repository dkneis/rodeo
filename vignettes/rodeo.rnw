...
\begin{shaded}
\begin{small}
<<showTableIdentifiers, echo=FALSE>>=
...
<<defForcingFunctions, echo=TRUE, eval=TRUE>>=
...
<<dataSingleSection, echo=TRUE, eval=TRUE>>=
...
<<generateSingleSectionR, echo=TRUE, eval=TRUE>>==
...
<<solveSingleSectionR, echo=TRUE, eval=TRUE, fig=TRUE>>=
library(deSolve)
...
<<initMultiSections, echo=TRUE, eval=TRUE>>=
# Instantiate new object
model <- new("rodeo", vars=subset(exampleIdentifiers,type=="v"),
  pars=subset(exampleIdentifiers,type=="p"),
  funs=subset(exampleIdentifiers,type=="f"),
  ...
  sections=3)
@
<<generateMultiSectionsR, echo=TRUE, eval=TRUE>>==
code <- model$generate(name="derivs",lang="r")
derivs <- eval(parse(text=code))
@
<<dataMultiSections, echo=TRUE, eval=TRUE>>=
vars <- cbind(
  c_z=seq(from=0, to=50, length.out=model$sections()),
  c_do=rep(9.022, model$sections()),
  v=rep(1.e6, model$sections()))
pars <- c(kd=5.78e-7, h_do=0.5, s_do_z=2.76, wind=1,
  depth=2, temp=20, q_in=1, q_ex=1)
pars <- matrix(rep(pars, model$sections()), nrow=model$sections(),
  byrow=TRUE, dimnames=list(NULL, names(pars)))  
model$assignPars(pars)
model$assignVars(vars)
@
<<solveMultiSectionsR, echo=TRUE, eval=TRUE, fig=TRUE, width=6, height=2.5>>=
out <- ode(y=model$queryVars(), times=t, func=derivs,
  parms=model$queryPars())
layout(matrix(1:model$sections(), nrow=1))
plot(out, which=paste("c_do",1:model$sections(),sep="."), mfrow=NULL)
@
...
out <- ode(y=model$queryVars(), times=t, func=lib["libFunc"],
  ...
  initfunc="initmod", nout=model$lenPros()*model$sections())
layout(matrix(1:model$sections(), nrow=1))
