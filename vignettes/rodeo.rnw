...
Using \software{rodeo}, the model can be described using just tabular text files (Tables~\ref{tab:vars} -- \ref{tab:stoi}). These files are shipped with the package and can be loaded with \software{R}'s \verb|data| method.
...
data(vars)
print(format(vars, justify="right"), row.names=FALSE)
@
...
library(rodeo, quietly=TRUE)
data(pars)
print(format(pars, justify="right"), row.names=FALSE)
@
\end{small}\end{shaded}
\end{table}
...
library(rodeo, quietly=TRUE)
data(funs)
print(format(funs, justify="right"), row.names=FALSE)
@
\end{small}\end{shaded}
\end{table}
...
library(rodeo, quietly=TRUE)
data(pros)
print(format(pros, justify="left"), row.names=FALSE)
@
\end{small}\end{shaded}
\end{table}

\begin{table}
\caption{Specification of stoichiometric factors. \label{tab:stoi}}
\begin{shaded}\begin{small}
<<showTableStoichiometry, echo=FALSE>>=
library(rodeo, quietly=TRUE)
data(stoi)
print(format(stoi, justify="left"), row.names=FALSE)
@
\end{small}\end{shaded}
\end{table}
