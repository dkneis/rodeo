...
A function to create a similar output as an R graphics has been written by Andew Dolman (see below). It needs the `ggplot2` package. Note that the symbols still not scale well with the size of the plot.

```{r plot_stoichio_matrix, eval=TRUE}
if ("ggplot2" %in% installed.packages()[,1]) {
  ...
  plot_stoichio_matrix= function(model, v, p) {
    ...
    #df2 <- tidyr::gather(df, variable, value, -process)
    df2= reshape(df, direction = "long", v.names="value", idvar="process",
      ids="process", timevar = "variable",
      times = (names(df)[-which(names(df)=="process")]),
      varying = list(names(df)[-which(names(df)=="process")]))
    row.names(df2)= NULL
...
A suitable Fortan code for (1) and (2) can be generated by the function call 'fortranWrapper_deSolve(NLVL)' where the integer argument `NLVL` specifies the desired number of spatial levels (default 1). The generated wrapper and initialization code for the considered case is shown below. For later use by a compiler, the generated code must be written to disk.

```{r fortranWrapper, echo=TRUE}
# Create and show wrapper code
fwrap= fortranWrapper_deSolve(nbox)
cat(fwrap)
