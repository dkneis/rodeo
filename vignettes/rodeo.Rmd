...
In general, there are two options how to deal with time-variable forcings:
...
What follows is a rather simple code allowing for use of the 'functions-of-time' approach in Fortran-based models. In order to use this, one must, for each forcing variable

 - Make a copy of the function 'forcing'.
 - Rename the copied function, i.e. replace the identifier 'forcing' by something appropriate (examples: 'temperature' or 'rateOfInflow').
 - Adjust the path to the corresponding data file, i.e. insert a valid path between the quotes.
 - Reference the forcing functions in the model definition (usually in the process rates). Supply them with the special argument `time` and with a dedicated value for signalling abnormal conditions (second argument named 'na'). In many situations, a very large negative number might be appropriate.
