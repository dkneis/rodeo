...
create_code= function(name, namesVars, namesPars, namesProc, ident,
  ...
  if (lang == "f95") {
    ...
    code=paste0(code,"subroutine ",name,"(time, ",ident["vecVars"],", ",ident["vecPars"],
      ", ",ident["lenLevels"],", ",ident["vecDrvs"],", ",ident["vecProc"],")",newline)
    ...
    code=paste0(code,"  integer, parameter:: ",ident["lenProc"],"=",nProc,newline)
    ...
    code=paste0(code,"  double precision, dimension(",ident["lenProc"],"*",ident["lenLevels"],"), intent(out):: ",ident["vecProc"],newline)
    ...
    code=paste0(code,"  function ",ident["vecProc"],"0D(",ident["levelIndex"],")",newline)
    code=paste0(code,"  implicit none",newline)
    code=paste0(code,"  ! Inputs",newline)
    code=paste0(code,"  integer, intent(in):: ",ident["levelIndex"],newline)
    code=paste0(code,"  ! Outputs",newline)
    code=paste0(code,"  double precision, dimension(",ident["lenProc"],"):: ",ident["vecProc"],"0D",newline)
    code=paste0(code,"  ! Local",newline)
    ...
    code=paste0(code,"  fun_",ident["vecProc"],"0D = function(",ident["levelIndex"],") {",newline)
    code=paste0(code,"  ",code_proc,newline)
    code=paste0(code,"  }",newline)
    code=paste0(code,"",newline)
    code=paste0(code,"  # Set vector of process rates (all spatial levels)",newline)
    code=paste0(code,"  ",ident["vecProc"]," = unname(fun_",ident["vecProc"],"0D(1:",ident["lenLevels"],"))",newline)
    code=paste0(code,"",newline)
    ...
    code=paste0(code,"  return(list(",ident["vecDrvs"],"=",ident["vecDrvs"],",",ident["vecProc"],"=",ident["vecProc"],"))",newline)
    code=paste0(code,"}",newline)
    return(code)
