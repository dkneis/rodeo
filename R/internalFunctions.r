...
namesOfFuns= function(obj) {
  ...
  string= paste(paste(obj$expr, collapse=";"),paste(obj$proc, collapse=";"),paste(unlist(obj$stox), collapse=";"), sep=";")
...
namesOfPars=  function(obj) {
  ...
  items[unlist(items) %in% names(obj$expr)]= NULL  # sort out names of expressions
